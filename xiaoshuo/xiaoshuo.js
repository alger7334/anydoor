...
const MAX_RT = 3;													//重试次数
const TEMP_DIR = './a/';											//临时目录
const NOVEL_FILE = './我只想安静的打游戏.txt';										//生成的文件
const CHAPTERS_URL = 'https://m.uctxt.com/book/30/30073/';		//目录地址
...
const CHAPTERS_CSS_SELECTOR = 'body > div#main >section.mb> dl.chapter-list > dd > a[href]';			//目录选择器
const CONTENT_CSS_SELECTOR = 'body > div#main > section#container > article >div#content';					//内容选择器
    ...
    return new Promise(async (resolve, reject) => {
        ...
        for (var i = 0; i < MAX_RT; i++) {
            ...
            content = await page.evaluate(async (selector, resultType, gap) => {
                ...
                result = await window.handleContent(result).catch(e => {
                    logd(e);
                });
            ...
            }, CONTENT_CSS_SELECTOR, CONTENT_RESULT_TYPE, GAP_EACH_CHAPTER).catch(ex => {
                logd(ex)
            });
        }
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (MULTI_PAGE) {
            ...
            for (var i = 0; i < urls.length; i++) {
                var url = urls[i];
                let content = await getPageContent(browser, url).catch(e => {
                    logd(e);
                });
                if (!content) {
                    reject();
            ...
            let content = await getPageContent(browser, chapterInfo.link).catch(e => {
                logd(e);
            });
            if (!content) {
                reject();
    ...
    return new Promise(async (resolve, reject) => {
        ...
        async function loop(browser) {
            ...
            while (true) {
                ...
                workingFlag++;
                var content = await getChapterContent(browser, chapter).catch(e => {
                    logd(e);
                });
...
async function run() {
    ...
    var chapterList;
    chapterList = await getNovelChapterList().catch(ex => {
        logd(ex);
    });
    if (!chapterList) {
    ...
    while (finishedFlag) {
        ...
        if (!DEBUG_1) {
            ...
            var proxyList;
            proxyList = await px.getProxyList().catch(ex => {
                logd(ex);
            });
            if (!proxyList) {
